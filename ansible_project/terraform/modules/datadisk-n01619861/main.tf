...
resource "azurerm_virtual_machine_data_disk_attachment" "n01619861-vmlinux-datadisk-attachment" {
  count              = length(var.vmlinux-datadisk-info.n01619861-vmlinux-names)
  managed_disk_id    = azurerm_managed_disk.n01619861-vmlinux-datadisk[count.index].id
  ...
  depends_on = [azurerm_managed_disk.n01619861-vmlinux-datadisk]
}
